//vito game machine. nvf 2k23, updated 2026

#include <Wire.h>
#include <Adafruit_SH110X.h>
#include "vito_pindefs.h"
#include "vito_bmps.h"

#define WHITE 1
#define BLACK 0

Adafruit_SH1106G display(128, 64, &Wire, oledRSTPin);
float gameSpeed = 3;
...
public:
  ...
  PongGame(int inleftBumperPin, int inrightBumperPin, int inbutton1Pin, int ingameSpeed)
    : leftBumperPin(inleftBumperPin), rightBumperPin(inrightBumperPin), button1Pin(inbutton1Pin) {
...
public:
  ...
  SongPlayer(int inleftBumperPin, int inrightBumperPin, int inbutton1Pin, int inbutton2Pin)
    : leftBumperPin(inleftBumperPin),
      rightBumperPin(inrightBumperPin), button1Pin(inbutton1Pin), button2Pin(inbutton2Pin) {
  }
  void drawVis() {
...
public:
  ...
  bool playGame() {
    while (1) {
      ...
      ball.x = map(1024 - analogRead(leftBumperPin), 0, 1023, ball.radius, display.width() - ball.radius - 1);
      ball.y = map(1024 - analogRead(rightBumperPin), 0, 1023, ball.radius, display.height() - ball.radius - 1);
      ball.draw();
...
public:
  ...
  String menuItems[4];  //The size of this array defines the size of the menu.
  int numItems;         //Be sure to set numItems accordingly
  int button1State;
  ...
  int show() {  // Display menu. Only rewrites the display on selection changes, which frees time for checking button input
